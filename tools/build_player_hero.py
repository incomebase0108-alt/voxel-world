# -*- coding: utf-8 -*-
# VOXEL WORLD - プレイヤーヒーロー作り込み版【方向確認用プレビュー】
...
out=os.path.join(work,"player_hero%s.glb"%(("_"+VARIANT) if VARIANT else ""))
...
print("[voxel] HERO export -> %s  %.3fMB  H%.2fm"%(out, sz/1048576, max(zs)))

# ---- プレビュー描画（front / 3q / idle-3q）----
for o in list(scene.objects):
    if o.animation_data: o.animation_data.action=None
    if o.animation_data:
        for trk in o.animation_data.nla_tracks: trk.mute=True   # rest立ち姿で描画
bpy.ops.object.light_add(type='SUN',location=(5,-7,9)); bpy.context.active_object.data.energy=4.2
bpy.ops.object.light_add(type='SUN',location=(-5,5,4)); bpy.context.active_object.data.energy=1.7
try: scene.render.engine='BLENDER_EEVEE_NEXT'
except Exception: scene.render.engine='BLENDER_EEVEE'
scene.render.resolution_x=620; scene.render.resolution_y=860
scene.world=scene.world or bpy.data.worlds.new("W"); scene.world.use_nodes=True
scene.world.node_tree.nodes["Background"].inputs[0].default_value=(0.52,0.6,0.72,1)
zmax=max(zs); look=(0,0.05,zmax*0.55)
def shot(name,loc):
...
shot("hero_player_front.png",(0.0, zmax*2.6, zmax*0.62))
shot("hero_player_3q.png",  (zmax*1.9, zmax*1.7, zmax*0.72))
print("[voxel] hero preview done")
