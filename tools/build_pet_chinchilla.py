...
VARIANTS = {
    ...
    'violet':   ((0.60,0.56,0.64),(0.94,0.93,0.95),(0.90,0.74,0.78),(0.80,0.60,0.66),(0.90,0.66,0.72),(0.62,0.10,0.16),(0.42,0.05,0.10),(0.90,0.90,0.94),(0.50,0.47,0.55),(0.94,0.93,0.96)),
...
def build_parts(C):
    ...
    # 前足（持つポーズ：腕を前に曲げ、両手を口(マズル)の直下で中央に揃える）
    for sx in (-1, 1):
        # 前腕（毛色・肘を前に出し、手先を内側＝中央上へ向ける）
        ellipsoid(sx*0.090, 0.350, 0.212, 0.050, 0.106, 0.057, FUR,
                  seg=14, ring=10, rotZ=sx*0.42, rotX=0.34, fur=0.007)
        # 手のひら（口の直下・中央寄りで軽く合わせる）
        ellipsoid(sx*0.045, 0.432, 0.262, 0.044, 0.040, 0.048, PAW, seg=14, ring=9, fur=0.003)
        # 指4本（上前方へ向け、正面から見える）
        for fx in (0.018, 0.040, 0.062, 0.083):
            ellipsoid(sx*fx, 0.466, 0.278, 0.013, 0.024, 0.015, PAW,
