...
```
- **加入**（id が新規出現）→「🤝 〇〇 が仲間になった！」トーストを自動表示。
- **離脱**（id が消える）→「💨 〇〇 が離脱した」トーストを自動表示。
- HP・order は毎フレームこのスナップショットで追従します。

### 指示口（実装してください・1行）
指示ボタン押下で ui.js が呼びます。AI/移動の実体は core 側：
```js
window.VoxelGame.commandCompanion = (id, order) => { /* order: 'follow'|'wait'|'attack' を該当仲間に適用 */ };
```
> 未実装でもボタンは表示されます（押しても何も起きないだけ・無害）。`state().companions[].order` を返せば点灯状態は同期します。

### 任意の push 口（state に companions を出さない実装向け・併用可）
```js
window.onCompanionJoin({ id, name });   // 加入トースト（id で重複抑止）
window.onCompanionLeave({ id, name });  // 離脱トースト
// 簡易別名: window.UI.companionJoin(c) / window.UI.companionLeave(c)
```
> ステータス常時表示は **state().companions が必須**（push はトーストのみ）。両方使っても id で二重トーストは抑止されます。
...
`ui.js` がタッチ端末でのみ（`navigator.maxTouchPoints>0` または `ontouchstart` で判定）仮想スティック＋ボタンを表示します（PCは非表示・操作不変）。

レイアウト: **左下＝移動パッド**（仮想スティック）/ **画面右半分スワイプ＝視点回転**（左半分はパッド用に空ける）/
**右下＝大型の攻撃ボタン**（タップ＝攻撃・破壊、長押し＝弓溜め＝充填リング点灯）/ ジャンプ・設置・ダッシュ・🎒インベントリ・⏸メニュー。
**装備はインベントリ画面をタップで操作**（DOMパネルなのでタッチでそのまま動作）。
